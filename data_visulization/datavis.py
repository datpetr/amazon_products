...
def plot02():
    ...
    ax.set_title("Correlation of Revenue and Advertising Cost for Each Product",y=1.01, fontsize = 15, fontweight = "bold") # Setting the labels and titles.
...
def plot05():
    ...
    for axs, location in zip(ax.axes.flatten(), amazon["Location"].unique()):
        ...
        axs.axhline(mean_price, color="black", linestyle="dashed", linewidth=2, alpha=0.2, label="Mean Price")
        axs.axvline(mean_units, color="black", linestyle="dashed", linewidth=2, alpha=0.2, label="Mean Units Sold") # To axtually plot the mean lines.
        
        axs.scatter(mean_units, mean_price, color="black", marker="o", s=30, label="Mean Marker", alpha=0.8) # To put a point where the lines coincide.
        
        axs.text(mean_units+3.5, mean_price+3.5, "Mean", ha="left", va="bottom", fontsize=13, color="black", alpha=0.5, fontweight="bold") # The text.
...
def plot08():
    ...
    amazon_p08 = amazon_year[amazon_year['Year'].isin([2019,2020, 2021])] # This is to exclude the years that didn't have enough data to draw inferences form.
    ...
    ax.set_title("Advertising Costs of Categories (2019-2021)",fontsize = 15, fontweight = "bold") # Setting the labels and titles.
...
def plot09():
    ...
    ax.set_xlabel("")
    ax.set_title("Distribution of Customer Ratings by Shipping Method", fontsize=15,fontweight="bold") #Setting the labels and titles.
...
def plot11():
    ...
    ax.set_title("Discount Rates of Items by Shipping Method Used",fontsize=15,fontweight="bold") # Setting labels and titles.
...
def plot13():
    ...
    for i, year in enumerate(amazonmod.columns[1:]):
        if bottom_position is None:
            bottom_position = 0
        ax.bar(amazonmod['Category'], amazonmod[year], label=str(year), alpha=0.8, color=the_colors[i], bottom=bottom_position, linewidth=0)
    ...
    pozs = [[0, 120], [0, 340], [0, 570], [1, 220], [1, 780], [1, 1360], [2, 750], [2, 1720], [2, 2500], [3, 80], [3, 250], [3, 420], [4, 60], [4, 200], [4, 370]]
    
    for i, val in enumerate(vals):
        ax.text(pozs[i][0], pozs[i][1], s=str(val), ha="center", va="center", fontsize=10, color="white", alpha=0.95, fontweight="bold")
